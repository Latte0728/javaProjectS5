...
<div class="container">
  <div class="text-center">
    <button type="button" onclick="location.href='${ctp}/reservation/reservationAdminView?yy=${yy-1}&mm=${mm}';" class="btn btn-secondary btn-sm" title="이전년도">◁◁</button>
    <button type="button" onclick="location.href='${ctp}/reservation/reservationAdminView?yy=${yy}&mm=${mm-1}';" class="btn btn-secondary btn-sm" title="이전월">◀</button>
    <font size="5">${yy}년 ${mm+1}월</font>
    <button type="button" onclick="location.href='${ctp}/reservation/reservationAdminView?yy=${yy}&mm=${mm+1}';" class="btn btn-secondary btn-sm" title="다음월">▶</button>
    <button type="button" onclick="location.href='${ctp}/reservation/reservationAdminView?yy=${yy+1}&mm=${mm}';" class="btn btn-secondary btn-sm" title="다음년도">▷▷</button>
    &nbsp;&nbsp;
    <button type="button" onclick="location.href='${ctp}/reservation/reservationAdminView';" class="btn btn-secondary btn-sm" title="오늘날짜"><i class="fa fa-home"></i></button>
  </div>
  <br/>
  <div class="text-center">
          ...
          <td id="td${cnt}" ${todaySw==1 ? 'class=today' : ''} style="font-size:0.9em">
            ...
            <a href="scheduleMenu?ymd=${ymd}">${st.count}<br/>
	            ...
	            <c:forEach var="vo" items="${vos}">
	              <c:if test="${fn:substring(vo.WDate,8,10)==st.count}">
		              ☆ ${vo.reservationCnt} 건<br/>
	              </c:if>
	            </c:forEach>
            </a>
          </td>
